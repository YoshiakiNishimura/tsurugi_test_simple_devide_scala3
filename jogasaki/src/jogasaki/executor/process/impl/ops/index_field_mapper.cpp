...
status index_field_mapper::process(
    ...
    timings->index_field_mapper_process += std::chrono::duration_cast<metrics::decode_timings::duration_type>(std::chrono::steady_clock::now() - start);
    return populate_field_variables(k, v, target, resource/*,timings*/);
}
...
status index_field_mapper::populate_field_variables(
    ...
    //timings->populate_field_variables += std::chrono::duration_cast<metrics::decode_timings::duration_type>(std::chrono::steady_clock::now() - start);
   ...
   // timings->decode_fields += std::chrono::duration_cast<metrics::decode_timings::duration_type>(std::chrono::steady_clock::now() - start1);
    return status::ok;
}
