...
struct decode_timings {
    ...
    std::chrono::milliseconds start_ms           = duration_type::zero();
    std::chrono::milliseconds end_ms             = duration_type::zero();
    std::chrono::milliseconds duration_ms        = duration_type::zero();
    ...
    void dump_csv(std::ostream& out) const noexcept {
        ...
        out << start_ms.count() << "," << end_ms.count() << "," << duration_ms.count() << ","
            ...
            << yield_check.count() << "," << loop_count << std::endl;
    }
};
